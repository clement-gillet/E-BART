...
class GenerationMixin:
    ...
    def _prepare_model_inputs(
        ...
        return inputs, guidance, input_name, model_kwargs
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, inputs_tensor: torch.Tensor, guidance: torch.Tensor, model_kwargs, model_input_name: Optional[str] = None
    ) -> Dict[str, Any]:
        # 1. get encoders
        encoder_x, encoder_g = self.get_encoder()

        # Compatibility with Accelerate big model inference: we need the encoders to output stuff on the same device
        # as the inputs.
        if hasattr(encoder_x, "_hf_hook"):
            encoder_x._hf_hook.io_same_device = True

        if hasattr(encoder_g, "_hf_hook"):
            encoder_g._hf_hook.io_same_device = True

        # 2. Prepare encoders' args and encoders' kwargs from model kwargs.
        ...
        encoder_x_signature = set(inspect.signature(encoder_x.forward).parameters)
        encoder_x_accepts_wildcard = "kwargs" in encoder_x_signature or "model_kwargs" in encoder_x_signature
        if not encoder_x_accepts_wildcard:
            encoder_x_kwargs = {
                argument: value for argument, value in encoder_x_kwargs.items() if argument in encoder_x_signature
            }
        ...
        if not encoder_g_accepts_wildcard:
            encoder_g_kwargs = {
                argument: value for argument, value in encoder_g_kwargs.items() if argument in encoder_g_signature
            }

        # 3. make sure that encoder returns `ModelOutput`
        model_input_name = model_input_name if model_input_name is not None else self.main_input_name
        encoder_x_kwargs["return_dict"] = True
        encoder_g_kwargs["return_dict"] = True
        encoder_x_kwargs[model_input_name] = inputs_tensor
        encoder_g_kwargs[model_input_name] = guidance
        model_kwargs["x_encoder_outputs"]: ModelOutput = encoder_x(**encoder_x_kwargs)
        model_kwargs["guidance"]: ModelOutput = encoder_g(**encoder_g_kwargs)
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if is_encoder_decoder:
            if model_kwargs.get("x_encoder_outputs") is None:
                raise ValueError("If `is_encoder_decoder` is True, make sure that `encoder_outputs` is defined.")
            model_kwargs["x_encoder_outputs"] = _expand_dict_for_generation(model_kwargs["x_encoder_outputs"])
            model_kwargs["guidance"] = _expand_dict_for_generation(model_kwargs["guidance"])
    ...
    def generate(
        ...
        **kwargs,
        ...
        inputs_tensor, guidance, model_input_name, model_kwargs = self._prepare_model_inputs(
            inputs, g, generation_config.bos_token_id, model_kwargs
        )
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                inputs_tensor, guidance, model_kwargs, model_input_name
            )
